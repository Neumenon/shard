...
fn truth_duplicate_entry_names_rejected() {
    ...
    let buf = w.to_bytes();
    match ShardV2Reader::from_bytes(buf) {
        Ok(r) => {
...
fn truth_zero_length_entry_valid() {
    ...
    let r = ShardV2Reader::from_bytes(buf).expect("zero-length entry shard should be valid");
...
fn truth_unicode_entry_name() {
    ...
    let r = ShardV2Reader::from_bytes(buf).expect("unicode name shard should be valid");
    assert_eq!(r.entry_count(), 1);
    assert_eq!(
...
fn truth_bad_magic_rejected() {
    ...
    let result = ShardV2Reader::from_bytes(data);
...
fn truth_crc_mismatch_rejected() {
    ...
    let data = std::fs::read(&path).expect("read fixture");
    match ShardV2Reader::from_bytes(data) {
...
fn truth_empty_input_rejected() {
    let result = ShardV2Reader::from_bytes(vec![]);
