//! Streaming writer roundtrip tests for `ShardV2StreamWriter`.

use std::path::PathBuf;

use shard_format::{
    ShardV2Reader, ShardV2StreamWriter,
...
fn test_basic_roundtrip() {
    ...
    let mut sw = ShardV2StreamWriter::new(&path, ROLE_MOSH, 10).unwrap();
...
fn test_flag_streaming_is_set() {
    ...
    let mut sw = ShardV2StreamWriter::new(&path, ROLE_MOSH, 5).unwrap();
    sw.begin_data().unwrap();
...
fn test_role_propagated() {
    ...
    let mut sw = ShardV2StreamWriter::new(&path, ROLE_SAMPLE, 5).unwrap();
    sw.begin_data().unwrap();
...
fn test_many_entries() {
    ...
    let n: usize = 100;
    let mut sw = ShardV2StreamWriter::new(&path, ROLE_MOSH, n).unwrap();
    sw.begin_data().unwrap();
    for i in 0..n {
...
fn test_fewer_entries_than_max() {
    ...
    let mut sw = ShardV2StreamWriter::new(&path, ROLE_MOSH, 1000).unwrap();
    sw.begin_data().unwrap();
