...
static uint8_t* build_shard(const test_entry_t* entries, uint32_t n,
    ...
    for (uint32_t i = 0; i < n; i++) {
        ...
        uint64_t hash = shard_xxhash64(entries[i].name, strlen(entries[i].name));
        ...
        uint32_t crc = shard_crc32c(entries[i].data, entries[i].len);
...
TEST(path_parent_with_slash) {
    char buf[256];
    char* r = shard_path_parent("layer.0/weight", buf, sizeof(buf));
    assert(r != NULL);
...
TEST(path_parent_deep) {
    char buf[256];
    char* r = shard_path_parent("a/b/c", buf, sizeof(buf));
    assert(r != NULL);
...
TEST(path_parent_no_slash) {
    char buf[256];
    char* r = shard_path_parent("embed", buf, sizeof(buf));
    assert(r != NULL);
    assert(strcmp(r, "") == 0);
}

TEST(path_base_with_slash) {
    const char* r = shard_path_base("layer.0/weight");
    assert(r != NULL);
    assert(strcmp(r, "weight") == 0);
}

TEST(path_base_deep) {
    const char* r = shard_path_base("a/b/c");
    assert(r != NULL);
    assert(strcmp(r, "c") == 0);
}

TEST(path_base_no_slash) {
    const char* r = shard_path_base("embed");
    assert(r != NULL);
...
TEST(split_path_simple) {
    char** parts = NULL;
    char*  buf   = NULL;
    int n = shard_split_path("layer.0/weight", &parts, &buf);
    assert(n == 2);
...
TEST(split_path_deep) {
    char** parts = NULL;
    char*  buf   = NULL;
    int n = shard_split_path("a/b/c/d", &parts, &buf);
    assert(n == 4);
...
TEST(split_path_no_slash) {
    char** parts = NULL;
    char*  buf   = NULL;
    int n = shard_split_path("embed", &parts, &buf);
    assert(n == 1);
...
TEST(split_path_empty) {
    char** parts = NULL;
    char*  buf   = NULL;
    int n = shard_split_path("", &parts, &buf);
    assert(n == 0);
    assert(parts[0] == NULL);
    free(parts);
...
TEST(join_path_two_parts) {
    ...
    char* r = shard_join_path(parts, 2);
    assert(r != NULL);
...
TEST(join_path_single) {
    ...
    char* r = shard_join_path(parts, 1);
    assert(r != NULL);
    assert(strcmp(r, "embed") == 0);
    free(r);
}
...
TEST(join_path_multi) {
    ...
    char* r = shard_join_path(parts, 3);
    assert(r != NULL);
 ...
 * because shard_from_buffer borrows the pointer (owns_buf=false). */
static shard_reader_t* make_layered_reader(uint8_t** out_buf) {
    ...
    *out_buf = build_shard(entries, 4, 64, &len);
    return shard_from_buffer(*out_buf, len);
}
...
TEST(list_children_exact_prefix_with_slash) {
    uint8_t* buf;
    shard_reader_t* r = make_layered_reader(&buf);
    assert(r != NULL);
    uint32_t count = 0;
    char** children = shard_list_children(r, "layer.0/", &count);
...
TEST(list_children_partial_prefix) {
    ...
    assert(r != NULL);
    uint32_t count = 0;
    char** children = shard_list_children(r, "layer.", &count);
    assert(children != NULL);
    assert(count == 2);
...
TEST(list_children_nonexistent_prefix) {
    ...
    assert(r != NULL);
    uint32_t count = 0;
    char** children = shard_list_children(r, "nonexistent/", &count);
    assert(children != NULL);
    assert(count == 0);
    shard_list_children_free(children, count);
    shard_close(r);
...
TEST(list_children_deduplicated_directories) {
    ...
    shard_reader_t* r = shard_from_buffer(buf, len);
    assert(r != NULL);
...
TEST(list_children_hierarchical_three_levels) {
    ...
    /* Top-level */
    uint32_t count = 0;
    char** top = shard_list_children(r, "", &count);
    assert(top != NULL);
    assert(arr_contains(top, count, "a"));
    assert(arr_contains(top, count, "f"));
    shard_list_children_free(top, count);

    /* Under a/ */
    char** under_a = shard_list_children(r, "a/", &count);
    assert(under_a != NULL);
    assert(arr_contains(under_a, count, "b"));
    assert(arr_contains(under_a, count, "e"));
    shard_list_children_free(under_a, count);

    /* Under a/b/ */
    char** under_ab = shard_list_children(r, "a/b/", &count);
    assert(under_ab != NULL);
    assert(count == 2);
    assert(arr_contains(under_ab, count, "c"));
    assert(arr_contains(under_ab, count, "d"));
    shard_list_children_free(under_ab, count);
...
TEST(read_entry_prefix_first_n_bytes) {
    ...
    shard_reader_t* r = shard_from_buffer(buf, buf_len);
    assert(r != NULL);

    size_t out_size = 0;
    const uint8_t* got = shard_read_entry_prefix(r, 0, 5, &out_size);
    assert(got != NULL);
    assert(out_size == 5);
...
TEST(read_entry_prefix_full_entry) {
    ...
    size_t out_size = 0;
    const uint8_t* got = shard_read_entry_prefix(r, 0, PAYLOAD_LEN, &out_size);
    assert(got != NULL);
...
TEST(read_entry_prefix_exceeds_entry_length) {
    ...
    size_t out_size = 0;
    const uint8_t* got = shard_read_entry_prefix(r, 0, 100000, &out_size);
    assert(got != NULL);
    assert(out_size == PAYLOAD_LEN);
    assert(memcmp(got, PAYLOAD, PAYLOAD_LEN) == 0);
    shard_close(r);
...
TEST(read_entry_prefix_zero_bytes) {
    ...
    size_t out_size = 99;
    const uint8_t* got = shard_read_entry_prefix(r, 0, 0, &out_size);
    assert(got != NULL);
    assert(out_size == 0);
    shard_close(r);
    free(buf);
...
TEST(read_entry_prefix_one_byte) {
    ...
    size_t out_size = 0;
    const uint8_t* got = shard_read_entry_prefix(r, 0, 1, &out_size);
    assert(got != NULL);
    assert(out_size == 1);
    assert(got[0] == 'H');
    shard_close(r);
...
TEST(security_entry_count_too_large) {
    ...
    shard_reader_t* r = shard_from_buffer(buf, SHARD_HEADER_SIZE);
    assert(r == NULL);
    free(buf);
}
