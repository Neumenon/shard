/*
 * test_writer.c — Roundtrip tests for shard writer + reader.
 *
 * Tests:
 ...
 * Caller must shard_close() and free(path). */
...
static void test_single_entry(void) {
    ...
    int rc = shard_writer_add_entry(w, "greeting", data, 11);
...
static void test_multiple_entries(void) {
    ...
    shard_writer_t* w = shard_writer_new(ROLE_SAMPLE);
    check(w != NULL, "writer_new");
    if (!w) return;
    ...
    for (int i = 0; i < 3; i++) {
        int rc = shard_writer_add_entry(w, names[i], datas[i], lens[i]);
        char desc[64];
    ...
    check(shard_entry_count(r) == 3, "entry_count == 3");
    ...
    for (uint32_t i = 0; i < 3; i++) {
        ...
        size_t sz = 0;
        const uint8_t* got = shard_read_entry(r, i, &sz);
...
static void test_typed_entries(void) {
    ...
    shard_writer_add_entry_typed(w, "tensor", t_data, 16, CONTENT_TYPE_TENSOR);
    shard_writer_add_entry_typed(w, "config", j_data, 7,  CONTENT_TYPE_JSON);
    ...
    check(e0 && shard_content_type(e0) == CONTENT_TYPE_TENSOR,
          "entry[0] content_type == TENSOR");

    const shard_index_entry_t* e1 = shard_get_entry(r, 1);
    check(e1 && shard_content_type(e1) == CONTENT_TYPE_JSON,
...
static void test_empty_shard(void) {
    ...
    check(shard_entry_name(r, 0) == NULL, "entry_name(0) == NULL");
    check(shard_get_entry(r, 0) == NULL, "get_entry(0) == NULL");
...
static void test_alignment_none(void) {
    ...
    shard_writer_t* w = shard_writer_new(ROLE_MOSH);
    if (!w) { check(false, "writer_new"); return; }
    shard_writer_set_alignment(w, ALIGN_NONE);

    const uint8_t d1[] = {0xDE, 0xAD, 0xBE, 0xEF};
    const uint8_t d2[] = {0xCA, 0xFE};

    shard_writer_add_entry(w, "x", d1, 4);
    shard_writer_add_entry(w, "y", d2, 2);
    ...
    /* Verify data */
    size_t sz;
    const uint8_t* got0 = shard_read_entry(r, 0, &sz);
    check(got0 && sz == 4 && memcmp(got0, d1, 4) == 0, "entry[0] data correct");

    const uint8_t* got1 = shard_read_entry(r, 1, &sz);
...
static void test_alignment_16(void) {
    ...
    shard_writer_t* w = shard_writer_new(ROLE_MOSH);
    if (!w) { check(false, "writer_new"); return; }
    shard_writer_set_alignment(w, ALIGN_16);

    const uint8_t d1[] = {0x01};
    const uint8_t d2[] = {0x02, 0x03};
    shard_writer_add_entry(w, "tiny1", d1, 1);
    shard_writer_add_entry(w, "tiny2", d2, 2);
...
static void test_alignment_64(void) {
    ...
    shard_writer_set_alignment(w, ALIGN_64);

    const uint8_t d1[100] = {0xAA};
    const uint8_t d2[200] = {0xBB};
    shard_writer_add_entry(w, "blob1", d1, 100);
    shard_writer_add_entry(w, "blob2", d2, 200);
...
static void test_lookup(void) {
    ...
    const uint8_t d[] = {1};
    shard_writer_add_entry(w, "layer.0/weight", d, 1);
    shard_writer_add_entry(w, "layer.1/weight", d, 1);
    shard_writer_add_entry(w, "config",         d, 1);
    ...
    check(shard_lookup(r, "layer.0/weight") == 0, "lookup layer.0/weight == 0");
    check(shard_lookup(r, "layer.1/weight") == 1, "lookup layer.1/weight == 1");
    check(shard_lookup(r, "config")         == 2, "lookup config == 2");
    check(shard_lookup(r, "missing")        == -1, "lookup missing == -1");

    check(shard_has_entry(r, "config"), "has_entry(config) == true");
    check(!shard_has_entry(r, "nope"), "has_entry(nope) == false");

    /* read_entry_by_name */
    size_t sz;
    const uint8_t* got = shard_read_entry_by_name(r, "layer.1/weight", &sz);
...
static void test_checksum_verification(void) {
    ...
    shard_writer_add_entry(w, "payload", orig_data, sizeof(orig_data) - 1);

    char* path = make_tmpfile();
    if (!path) { shard_writer_free(w); return; }
    ...
    if (r) {
        size_t sz;
        const uint8_t* data = shard_read_entry(r, 0, &sz);
    ...
    {
        shard_reader_t* r2 = shard_open(path);
        if (r2) {
            const shard_index_entry_t* e = shard_get_entry(r2, 0);
            uint32_t expected = shard_crc32c(orig_data, sizeof(orig_data) - 1);
...
static void test_from_buffer(void) {
    ...
    shard_writer_t* w = shard_writer_new(ROLE_WSHARD);
    if (!w) { check(false, "writer_new"); return; }

    const uint8_t d[] = {0x11, 0x22, 0x33, 0x44};
    shard_writer_add_entry_typed(w, "signal", d, 4, CONTENT_TYPE_BLOB);

    char* path = NULL;
    shard_reader_t* r_file = write_and_open(w, &path);
    shard_writer_free(w);
    ...
    shard_reader_t* r_buf = shard_from_buffer(buf, buf_len);
    check(r_buf != NULL, "from_buffer");
    if (r_buf) {
        check(shard_entry_count(r_buf) == 1, "entry_count == 1");
        const char* name = shard_entry_name(r_buf, 0);
        check(name && strcmp(name, "signal") == 0, "entry_name == signal");

        const shard_index_entry_t* e = shard_get_entry(r_buf, 0);
        check(e && shard_content_type(e) == CONTENT_TYPE_BLOB, "content_type == BLOB");

        size_t sz;
        const uint8_t* got = shard_read_entry(r_buf, 0, &sz);
        check(got && sz == 4 && memcmp(got, d, 4) == 0, "data matches");

        const shard_header_t* h = shard_header(r_buf);
...
static void test_large_entries(void) {
    ...
    if (!large_data) { shard_writer_free(w); return; }

    for (size_t i = 0; i < large_sz; i++) {
        large_data[i] = (uint8_t)(i % 256);
    }

    int rc = shard_writer_add_entry_typed(w, "big_tensor", large_data, large_sz, CONTENT_TYPE_TENSOR);
...
static void test_wshard_pattern(void) {
    ...
    shard_writer_add_entry_typed(w, "signal/imu", sig, 600, CONTENT_TYPE_BLOB);
...
int main(void) {
    printf("=== shard writer roundtrip tests ===\n\n");
