/*
 * shard.c — C implementation of the Shard binary container format.
 *
...
static void serialize_header(const shard_header_t* h, uint8_t* buf) {
...
static bool parse_header(const uint8_t* buf, size_t buf_len, shard_header_t* h) {
    if (buf_len < SHARD_HEADER_SIZE) return false;
    memcpy(h->magic, buf, 4);
    if (memcmp(h->magic, SHARD_MAGIC, 4) != 0) return false;
    h->version = buf[4];
    if (h->version != SHARD_VERSION) return false;
    h->role               = buf[5];
...
static void serialize_entry(const shard_index_entry_t* e, uint8_t* buf) {
...
static void parse_entry(const uint8_t* buf, shard_index_entry_t* e) {
...
struct shard_reader {
    ...
    shard_index_entry_t* entries;   /* array of entry_count entries */
...
static shard_reader_t* reader_parse(shard_reader_t* r) {
    ...
    if (!safe_size_mul(sizeof(shard_index_entry_t), (size_t)n + 1, &entries_alloc)) goto fail;
    r->entries = (shard_index_entry_t*)malloc(entries_alloc);
...
shard_reader_t* shard_open(const char* path) {
    ...
    shard_reader_t* r = (shard_reader_t*)calloc(1, sizeof(*r));
...
int32_t shard_lookup(const shard_reader_t* r, const char* name) {
    ...
    uint64_t hash = shard_xxhash64(name, len);
    for (uint32_t i = 0; i < r->entry_count; i++) {
...
const uint8_t* shard_read_entry(const shard_reader_t* r, uint32_t i, size_t* out_size) {
    ...
    if (shard_is_compressed(e)) {
        ...
        if ((r->header.flags & FLAG_HAS_CHECKSUMS) || e->checksum != 0) {
            uint32_t computed = shard_crc32c(decompressed, result);
    ...
    if ((r->header.flags & FLAG_HAS_CHECKSUMS) || e->checksum != 0) {
        uint32_t got = shard_crc32c(ptr, sz);
...
const uint8_t* shard_read_entry_by_name(const shard_reader_t* r, const char* name, size_t* out_size) {
    ...
    return shard_read_entry(r, (uint32_t)idx, out_size);
}
...
const uint8_t* shard_read_entry_prefix(const shard_reader_t* r, uint32_t i,
    ...
    if (shard_is_compressed(e)) return NULL;
    size_t sz = (size_t)e->disk_size;
...
char** shard_list_children(const shard_reader_t* r, const char* prefix,
    ...
    for (uint32_t i = 0; i < n; i++) {
        ...
        if (slash) {
            ...
            if (!child) {
                shard_list_children_free(result, count);
                return NULL;
            }
...
shard_writer_t* shard_writer_new(uint8_t role) {
    shard_writer_t* w = (shard_writer_t*)calloc(1, sizeof(*w));
    if (!w) return NULL;
    w->role        = role;
...
static int writer_add_raw(shard_writer_t* w, const char* name,
...
static int writer_add(shard_writer_t* w, const char* name,
                      const uint8_t* data, size_t len, uint16_t ct) {
    /* Checksum computed here for uncompressed entries */
    uint32_t csum = shard_crc32c(data, len);
...
int shard_writer_add_entry_compressed(shard_writer_t* w, const char* name,
    ...
    uint32_t orig_checksum = shard_crc32c(data, len);
...
/*
 * Layout (mirrors the Go ShardWriter.Close() logic exactly):
 *
...
int shard_writer_write(shard_writer_t* w, const char* path) {
    ...
    /* Write index entries */
    for (uint32_t i = 0; i < n; i++) {
        shard_index_entry_t e;
        const char* nm = w->entries[i].name;

        e.name_hash   = shard_xxhash64(nm, strlen(nm));
...
shard_stream_writer_t* shard_stream_writer_new(const char* path,
    ...
    shard_stream_writer_t* sw =
        (shard_stream_writer_t*)calloc(1, sizeof(*sw));
    if (!sw) return NULL;
...
static int sw_write_entry_impl(shard_stream_writer_t* sw,
                               const char* name,
                               const uint8_t* data,
                               size_t len,
                               uint16_t content_type) {
    ...
    uint32_t checksum = shard_crc32c(data, len);
...
int shard_stream_writer_finalize(shard_stream_writer_t* sw) {
...

    {
        ...
        for (uint32_t i = 0; i < n; i++) {
            ...
            shard_index_entry_t ie;
            ie.name_hash   = shard_xxhash64(e->name, strlen(e->name));
